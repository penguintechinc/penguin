...
//! [`squawk_client::doh::DohClient`] and [`squawk_client::license::Validator`]
//! without a real network call.
//!
//! Extends the shape of `penguin-licensing`'s `tests/mock_server` (a bare
//! `tokio::net::TcpListener` rather than a mocking crate, so this crate's
//! `cargo tree -i ring` gate only ever accounts for squawk-client's own
//! dependency graph) with request recording — method, path+query string,
//! and headers — since the DoH client's tests need to assert on the
//! `Authorization` header and the exact URL/query string used, not just the
//! response side.
//!
...
/// optional `Content-Length` body — the body itself is discarded, only the
/// method/path/headers are kept), writes `response`, and closes the
/// connection. Returns `None` if the connection closed before a full
/// request line arrived (e.g. a bare TCP probe).
