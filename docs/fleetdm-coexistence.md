...
- `fleetd_version` — version string or `null`
- `osqueryd_present` — boolean
- `osqueryd_version` — version string or `null`
...
sudo journalctl -u osqueryd -n 50 -f
```
...
- Agent re-detects on startup; no restart needed if you install FleetDM after penguin starts
