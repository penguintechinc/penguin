...
//! [`HostServices`] implementation (mirroring Go's `FakeHostServices` in
//! `go-client/internal/modules/tobogganing/fake.go`) and a path-routed mock
//! HTTP server standing in for the manager (auth token/refresh/revoke and
...
/// A trivial in-memory [`SecretStore`], matching Go's `FakeSecretStore`.
#[derive(Default)]
...
/// A [`Logger`] that discards every record. Tests that need to assert on
/// log content build a [`RecordingLogger`] instead.
#[derive(Default)]
...
/// A [`LicenseChecker`] double reporting everything enabled at a fixed
/// tier — matches Go's `FakeLicenseChecker` default (`professional`, all
/// features on).
...
/// A minimal [`HostServices`] test double, matching Go's `FakeHostServices`.
/// `config` is a plain public field tests set directly before wrapping this
/// in an `Arc<dyn HostServices>`, exactly like Go's `host.config = yaml`.
pub struct FakeHost {
    pub config: Vec<u8>,
...
/// (matching `penguin-licensing`'s and `squawk-client`'s own mock servers)
/// rather than a mocking crate, and extended with routing since this
/// crate's `AuthManager` and tunnel-config fetch talk to several distinct
...
/// [`RouteMap`] shared between [`MockManager`] and its accept loop.
