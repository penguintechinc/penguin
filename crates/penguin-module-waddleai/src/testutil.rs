...
//! [`HostServices`] implementation and a path-routed mock HTTP server
//! standing in for the WaddleAI API — both mirror
//! `penguin_module_waddlebot::testutil`'s identical doubles.
//!
//! Only compiled for tests (`#[cfg(test)] mod testutil;` in `lib.rs`), so
//! nothing here ships in the built module. Not every item here is used by
//! every test in this crate — shared test infra, like
//! `squawk_client`'s own `tests/mock_http` — so dead-code analysis is
//! disabled crate-locally rather than per-item.
#![allow(dead_code)]
...
/// A [`Logger`] recording every emitted record as one rendered line, so a
/// test can assert on log content — chiefly proving the virtual key never
/// reaches a log line.
#[derive(Default)]
...
/// A [`LicenseChecker`] double reporting everything enabled.
...
/// `prometheus::Registry` — every test gets its own registry, so tests
/// never collide over duplicate collector names.
...
impl RecordedRequest {
    ...
    /// Parses the recorded body as JSON.
...
/// A minimal mock server standing in for the WaddleAI API: understands just
/// enough HTTP/1.1 to receive one request per connection and route it by
...
/// exercises. Mirrors `penguin_module_waddlebot::testutil::MockHub`.
...
impl MockServer {
    ...
    /// How many requests matched `method`/`path` so far.
